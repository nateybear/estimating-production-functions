...
f = @(theta) acfObjective(theta, phiPrev, phiCurr, lPrev, lCurr, kPrev, kCurr);

estimate = fminsearch(f, [globals.betaL globals.betaK]);
...
function obj = acfObjective(theta, phiPrev, phiCurr, lPrev, lCurr, kPrev, kCurr)
...
obj = size(moments, 1) * mean(moments,1) * ( cov(moments) \ mean(moments, 1)' );

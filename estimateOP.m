function estimate = estimateOP(data, globals)
...
f = @(theta) objectiveFun(theta, phiPrev, phiCurr, kPrev, kCurr, yCurr, lCurr, betaL);
...
function obj = objectiveFun(theta, phiPrev, phiCurr, kPrev, kCurr, yCurr, lCurr, betaL)
...
% first stage---in OP we can identify betaL here, and then the rest
% of the fitted value is absorbed in phiHat
function [ betaL, phiHat ] = firstStageEstimate(data, globals)

X = [data.lnLabor(:) data.lnKapital(:) data.lnInvestment(:)];
